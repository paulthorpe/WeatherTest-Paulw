...
var ChoosenTempSetting = "cel";
var ChoosenWindSetting = "mph"

//--------------------------------------------
//JQUERY Startup 
$(document).ready(
	function () {
	    $("#cel").bind("click", "cel", setTempSetting);
	    $("#fah").bind("click", "fah", setTempSetting);

	    $("#mph").bind("click", "mph", setWindSetting);
	    $("#kph").bind("click", "kph", setWindSetting);
...
//--------------------------------------------
function setTempSetting(tempsetting)
{
    ChoosenTempSetting = tempsetting.data;

    $(".tempSelector").css('color', 'black');
    $("#" + ChoosenTempSetting).css('color', 'red');
    return false;
}

function setWindSetting(windsetting) {
    ChoosenWindSetting = windsetting.data;

    $(".windSelector").css('color', 'black');
    $("#" + ChoosenWindSetting).css('color', 'red');
    return false;
}
...
function init()
{
    //get Temp types for user selection
	//initalise Infomap
	CreateTeamDetails();
	Outlook();	
}
...
function ShowTeamDetails(lTeamIndex)
{
    ...
    //MAKE call to service to collect the Weather Data
	$.get("api/weather/" + elm[lTeamIndex].Name + "/" + ChoosenTempSetting + "/" + ChoosenWindSetting, function (data) {
	    var weatherData = JSON.parse(data)
	    $("#Stadium").text("Weather Data:");
	    Capacity.innerHTML = "Temp : " + parseFloat(weatherData.temperature).toFixed(2);
	    Formed.innerHTML = "Wind : " + parseFloat(weatherData.windSpeed).toFixed(2);
	    Manager.innerHTML = "WindSpeed: " + ChoosenWindSetting;
	    Pos.innerHTML = "Temp: " + ChoosenTempSetting;
	});
